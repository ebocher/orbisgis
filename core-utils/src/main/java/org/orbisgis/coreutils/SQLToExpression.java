...
public class SQLToExpression extends ExpressionDeParser {
    ...
    public static Expression transform(String selectExpression)  {
        ...
        return sqlToExpression.parseSingleExpression(selectExpression);
    }
    ...
    public static Map<String, Expression> toExpressions(String... selectExpression)  {
        ...
        for(int i = 0; i < selectExpression.length; i++) {
            Expression exp = sqlToExpression.parseSingleExpression(selectExpression[i]);
            if(exp!=null){
    ...
    /**
     * Build geotools definitions from a select expression form
     * @param selectExpression
     * @return
     */
    public static List<Definition> convertToDefinition(String selectExpression) {
        try {
            if (selectExpression != null && !selectExpression.isEmpty()) {
                SQLToExpression sqlToExpression = new SQLToExpression();
                Select select = (Select) CCJSqlParserUtil.parse("SELECT " + selectExpression);
                String alias_tmp = "exp_";
