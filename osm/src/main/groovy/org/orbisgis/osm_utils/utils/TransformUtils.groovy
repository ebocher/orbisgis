 ...
 * @param sql             Groovy sql obkect to manage database containing the OSM data. (Mandatory)
 ...
 */
static def toPolygonOrLine(String type, Sql sql, osmTablesPrefix, epsgCode = 4326, tags, columnsToKeep) {
    ...
    buildIndexes(sql, osmTablesPrefix)

    def outputWay = waysProcess(sql, osmTablesPrefix, epsgCode, tags, columnsToKeep)
    def outputRelation = relationsProcess(sql, osmTablesPrefix, epsgCode, tags, columnsToKeep)

    if (outputWay && outputRelation) {
        //Merge ways and relations
        def columnsWays = getColumnNames(sql.connection, TableLocation.parse(outputWay, DBTypes.H2))
        def columnsRelations = getColumnNames(sql.connection, TableLocation.parse(outputRelation, DBTypes.H2))
        ...
        sql.execute "ALTER TABLE $outputWay RENAME TO $outputTableName".toString()
        info "The way $type have been built."
    } else if (outputRelation) {
        sql.execute "ALTER TABLE $outputRelation RENAME TO $outputTableName".toString()
 ...
 */
static def buildIndexes(Sql sql, String osmTablesPrefix){
    if (!sql) {
        error "Invalid database connection"
        return false
    }
